...
# Canonical triple format: [[A]] --relation--> [[B]]
...
def extract_triples(pages_written: list[str], wiki_dir: Path = WIKI_DIR) -> int:
    """Extract entity→relation→entity triples from newly written pages and append to ontology.

    Called by the compiler after every ingest. Append-only — never rewrites the ontology.
    Returns the count of new triples appended.
    """
    ensure_graph_dirs()
    existing = _load_existing_triples()

    # Collect content of newly written pages
    pages_content_parts: list[str] = []
    total_chars = 0
    for entry in pages_written:
        # entry format: "page_type/Title String"
        if "/" not in entry:
            continue
    ...
    # Build list of existing entity names so LLM uses consistent naming
    entity_names: list[str] = []
    for triple in existing:
        m = TRIPLE_RE.match(triple)
        if m:
            entity_names.extend([m.group("a"), m.group("b")])
    unique_entities = sorted(set(entity_names))[:100]  # cap to avoid huge prompts

    prompt = (
        "You are an ontology extractor. Extract entity→relation→entity triples from these wiki pages.\n\n"
        + (
            f"Existing entity names — use EXACTLY these spellings if the entity appears:\n"
    ...
    for item in items:
        ...
        if not a or not rel or not b:
            continue
        if "-->" in rel:  # prevent parse corruption
            continue
        formatted = f"[[{a}]] --{rel}--> [[{b}]]"
        if formatted not in existing:
            new_triples.append(formatted)
            existing.add(formatted)  # dedup within this batch too
...
def analyze() -> str:
    """Run graph analysis on the ontology: centrality, gaps, research questions.

    Writes gap questions to data/todos/gaps.md and returns a full report string.
    """
    ensure_graph_dirs()
    existing = _load_existing_triples()

    if not existing:
        return "Ontology is empty — ingest some sources first, then run `second-brain graph`."
    ...
    # Bridges in undirected view — conceptual weak links
    try:
    ...
    # High in-degree = things many concepts point to (core hubs)
    in_degree = sorted(G.in_degree(), key=lambda x: x[1], reverse=True)[:5]  # type: ignore
    # High out-degree = concepts that connect to many things
    ...
    # LLM call: generate research questions from the analysis
    ontology_sample = "\n".join(list(existing)[-150:])  # most recent 150 triples
    gap_prompt = (
        f"Based on this knowledge graph analysis, generate 7-10 concrete research questions "
        f"that would fill the most important knowledge gaps.\n\n"
        f"Focus on:\n"
        f"- Isolated clusters that should connect to the main graph\n"
        f"- Central concepts that likely need deeper exploration\n"
        f"- Missing bridges between major topic areas\n\n"
        f"Analysis:\n{report}\n\n"
...
def visualize(output_path: Path | None = None) -> Path:
    ...
    Nodes are sized and coloured by betweenness centrality:
      red/large  → high-centrality hubs
      orange     → medium centrality
      blue/small → peripheral concepts

    Hovering a node shows its name, centrality score, and connection count.
    Hovering an edge shows the relation label.
    ...
    net = Network(
        ...
        font_color="#e0e0e0",
        notebook=False,
        cdn_resources="in_line",  # fully self-contained HTML, no external CDN needed
    )
    net.from_nx(G)

    # Style nodes by centrality tier
    for node in net.nodes:
        ...
        if c > 0.08:
            node["color"] = "#ff6b6b"   # red — key hub
            node["size"] = 42
            node["font"] = {"size": 17, "bold": True, "color": "#ffffff"}
        elif c > 0.02:
            node["color"] = "#ffa94d"   # orange — medium
            node["size"] = 26
            node["font"] = {"size": 13, "color": "#ffffff"}
        else:
            node["color"] = "#74c0fc"   # blue — peripheral
            node["size"] = 14
